  ...
  componentDidMount() {
    ...
    fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=${apiKey}`)
      ...
      .then(data => {
        ...
        //basicamente me fij si esta en fav como hice con las otras js
      ...
      <div className='detalle-container'>
        ...
        {/* tener esto en cuenta para cuando me dice lo de "Cannot read properties of null (reading 'title')" */}
        {
          ...
          <>
            ...
            <p>Fecha de estreno:{peli.release_date}</p>
            <p>Duración: {peli.runtime} minutos</p>
            <p>Sinopsis: {peli.overview}</p>

            {
              this.state.favorito ?
              <button onClick={() => this.sacarDeFav(peli.id)}>Quitar de favoritos</button>
              :
              <button onClick={() => this.agregarAFav(peli.id)}>Agregar a favoritos</button>
            }
